    ...
    <div class="pagetitle">
      ...
      <nav class="d-flex align-items-center justify-content-between">
        ...
        <div class="d-flex justify-content-between align-items-center same-cstm">
                  <a style="height:35px;" class="bg text-white p-2 rounded" href="{{route('addInterns')}}">{{ __('apilang.add') }}</a>
                </div>
      </nav>
    </div><!-- End Page Title -->
              ...
              <!-- Table with stripped rows -->
                  <div class="card-body">
                      ...
                      <form>
                        ...
                        <tbody>
                            ...
                            <tr style="border:2px solid #eee; padding:8px;">
                                ...
                                <td>{{ $intern->name }}</td>
                                <td>{{ $intern->email }}</td>
                                <td>{{ isset($intern->sections) ? $intern->sections[0]->name: '' }}</td>
                                <td>{{ $intern->status == 0 ? 'New' : ($intern->status == 1 ? 'Active' : ($intern->status == 2 ? 'Inactive' : 'Discharge')) }}</td>
                                <td>{{ date('j M, Y', strtotime($intern->created_at)) }}</td>

                                <td style="padding-left:5px;">                  
                                  <span class="badge text-bg-warning py-2"><a class="text-white" href="{{route('internView',$intern->id)}}">{{ __('apilang.view') }}</a></span>
                                  <span class="badge text-bg-danger bg-danger py-2 text-white" style="cursor: pointer;" onclick="deleteConfirmation('{{ route("internDelete", $intern->id)}}')">{{ __('apilang.delete') }}</span>
                                  <span class="badge bg text-white py-2"><a class="text-white" href="{{route('internUpdate',$intern->id)}}">{{ __('apilang.update') }}</a></span>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    ...
                    <div class="pagination-wrap">
                        <div class="pageintaion-count my-2">Displaying {{$interns->count()}} of {{ $interns->total() }} User(s).</div>
                        <div class="pageintaion-pages">{!! $interns->appends(Request::except('page'))->render() !!}</div>
                    </div>                    
                </div>
